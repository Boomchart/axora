...
class AdminOrderExport implements FromQuery, WithHeadings, WithMapping, WithChunkReading, WithColumnFormatting, ShouldQueueContract
{
    ...
    {
        $query = DB::table('card_issueds as t')
    ...
    {
        return ['Reference', 'Name', 'Amount', 'Currency', 'Status', 'Customer Name', 'Customer Email', 'Customer Phone',  'Date'];
    }
    ...
    public function map($transaction): array
    {
        $baseResult = [
            $transaction->id,
            ucwords($transaction->card_name),
            sprintf('%.2f', $transaction->amount),
            $transaction->card_currency,
