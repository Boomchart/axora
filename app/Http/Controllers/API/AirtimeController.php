...
use App\Models\{Settings, Transactions, CountryReg, AirtimeProvider};
...
class AirtimeController extends Controller
{
    ...
    public function operatorsByCountry(Request $request, string $country)
    {
        ...
        if ($this->access == true) {
            ...
            $resource = AirtimeProvider::whereIso2($country)->orderBy('title', 'asc')
            ...
            $apiresponse = [
                'message' => __('Operators'),
                'status' => 'success',
                'data' => AirtimeResource::collection($resource)->map(fn($resource) => new AirtimeResource($resource, $this->client))
            ];
            ...
            try {
                ...
                if ($airtime) {
                    if ($airtime->only_denominations) {
                        ...
                        if ($request->amount < $airtime->min) {
                            $msg = __('Amount is lower than minimum amount of ') . $airtime->min;
                            $apiresponse = ['message' => $msg, 'errors' => ['amount' => [$msg]], 'status' => 'failed', 'data' => null];
                            $this->logError(422, $apiresponse);
                            return response()->json($apiresponse, 422);
                        }
                        if ($request->amount > $airtime->max) {
                            $msg = __('Amount is higher than maximum amount of ') . $airtime->max;
                            $apiresponse = ['message' => $msg, 'errors' => ['amount' => [$msg]], 'status' => 'failed', 'data' => null];
                            $this->logError(422, $apiresponse);
                            return response()->json($apiresponse, 422);
                        }
                ...
                if ($airtime->charge_phase == 'before_conversion') {
                    $our_charge = (float) calculateFee(removeCommas($amount / $airtime->rate), 'both', $our_flat_fee, $our_percent_fee) * $airtime->rate;
                    ...
                    $our_charge = (float) calculateFee(removeCommas($amount), 'both', $our_flat_fee, $our_percent_fee);
                    $vendor_charge = (float) calculateFee(
                        removeCommas($amount),
                        'both',
                ...
                $item = [
                    ...
                    'operator' => new AirtimeResource($airtime, $this->client)
                ];

                $apiresponse = ['message' => __('Airtime Quote calculated'), 'status' => 'success', 'data' => $item];
                $this->logError(200, $apiresponse);
                return response()->json($apiresponse, 200);
            } catch (\Exception $e) {
    ...
    public function order(Request $request)
    {
        ...
        $operators = AirtimeProvider::whereStatus(1)->whereIn('id', $operatorIds)->get()->keyBy('id');
        ...
        try {
            ...
            foreach ($data as $key => $value) {
                ...
                if ($operator->only_denominations) {
                    ...
                    if ($value['amount'] < $operator->min) {
                        $msg = __('Amount is lower than minimum amount of ') . $operator->min;
                        $this->updateError($rowKey, ['message' => $msg, 'errors' => ['amount' => [$msg]], 'status' => 'failed', 'data' => null], 422);
                        $hasErrors = true;
                        continue;
                    }
                    if ($value['amount'] > $operator->max) {
                        $msg = __('Amount is higher than maximum amount of ') . $operator->max;
                        $this->updateError($rowKey, ['message' => $msg, 'errors' => ['amount' => [$msg]], 'status' => 'failed', 'data' => null], 422);
                        $hasErrors = true;
                        continue;
                    }
