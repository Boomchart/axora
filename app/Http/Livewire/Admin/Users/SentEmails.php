...
class SentEmails extends Component
{
    ...
    public function deleteAll()
    {
        ...
        $this->reset(['archive']);
        $this->emit('saved');
        $this->emit('success', __('Messages deleted'));
        $this->emit('clearMarkAll');
        $this->emit('closeModal', 'deleteall');
    }
    ...
    {
        $this->message = SentEmail::whereContactId($this->client->contact_id)->when($this->search, function ($query) {
            $this->emit('drawer');
        ...
        $this->checked();
        return view('livewire.admin.users.sent-emails', [
