 ...
 (function ($, window, document, undefined) {
	...
	$.extend(Plugin.prototype, {
		...
		buildInputBoxes: function () {
			...
			let currentValue = [];
			let placeholders = [];
			let touchplaceholders = (this.settings.inputs == 4) ? "Enter Pin" : "Enter Code";  // in touch mode we have just 1 big input box, and there is only 1 placeholder in this case
			...
			if (this._isTouchDevice()) {
				...
				let input = $('<input>').attr({
					...
					'autocomplete': 'off',
					'autofocus': 'on'
				}).addClass(this.settings.inputclass + ' form-control-lg pincode-input-text bg-token-mobile fs-1').appendTo(wrapper);
				...
				for (let i = 0; i < this.settings.inputs; i++) {
					if (i == (this.settings.inputs - 1)) {
						$('<div/>').addClass('touch-flex-cell rounded me-4 bg-light fw-bold fs-1').addClass('last').appendTo(touchtable);
					} else {
						$('<div/>').addClass('touch-flex-cell rounded me-4 bg-light fw-bold fs-1').appendTo(touchtable);
					}
				}
				if (this.settings.hidedigits) {
					// hide digits
				...
				for (let i = 0; i < this.settings.inputs; i++) {
					...
					}).addClass(this.settings.inputclass + ' form-control-lg  pincode-input-text').appendTo(this._container);
					...
					if (i == 0) {
						input.attr({
							'autofocus': 'on'
						}).addClass('first rounded me-4 bg-token fw-bold fs-1');
					} else if (i == (this.settings.inputs - 1)) {
						input.addClass('last rounded bg-token fw-bold fs-1');
					} else {
						input.addClass('mid rounded me-4 bg-token fw-bold fs-1');
					}
