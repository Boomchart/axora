<div>
    <div class="toolbar" id="kt_toolbar">
        <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
            <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                <h1 class="text-dark fw-bolder my-1 fs-2"> {{$country->name}} {{__('Gift Cards')}} ({{number_format_short($cards->total())}})</h1>
            ...
            <div class="card-header pe-5 border-0">
                <div class="card-title">
                    <div class="d-flex justify-content-center flex-column me-3">
                        <div class="fs-4 text-gray-900 text-hover-danger me-1 lh-1">{{__('Create a Card')}}</div>
                    </div>
                </div>
                ...
                <div class="pb-5 mt-10 position-relative zindex-1">
                    <form class="form w-100 mb-10" wire:submit.prevent="addCard" method="post">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="fv-row mb-6">
                                    <label class="form-label text-dark fs-7">{{__('Card Art')}}</label>
                                    <input class="form-control form-control-solid" type="file" wire:model="image" id="art" accept="{{allowedImageTypes()}}" />
                                    <div wire:loading wire:target="image">{{__('Uploading card art')}}...</div>
                                    @error('image')
                                    <span class="form-text text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                                <div class="fv-row mb-6">
                                    <label class="form-label text-dark fs-7 required">{{__('Card Category')}}</label>
                                    <select class="form-select form-select-solid" wire:model="category" required multiple size="10">
                                ...
                                <div class="fv-row mb-6">
                                    ...
                                    <select class="form-select form-select-solid" wire:model="provider">
                                        ...
                                        <option value="redboxx">{{__('Redboxx')}}</option>
                                ...
                                <div class="fv-row mb-6">
                                    <label class="form-label text-dark fs-7">{{__('Vendor Gift Card')}}</label>
                                    ...
                                    <span class="form-text text-danger">{{$message}}</span>
                                    @enderror
                                    <div wire:loading wire:target="provider" class="axora-kyc-loading">{{ __('Fetching Gift Cards') }}...</div>
                                </div>
                                <div class="fv-row mb-6">
                                    <label class="form-label text-dark fs-7 required">{{__('Card Title')}}</label>
                                    <input class="form-control form-control-solid" type="text" wire:model="title" required placeholder="{{__('Title of card')}}" />
                                    @error('title')
                                    <span class="form-text text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="p-5 border rounded-4 mb-10">
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-6">
                                            <p class="fs-8 mb-0">{{__('Amount')}} {{$loop->iteration}}</p>
                                        </div>
                                    </div>
                                    <div class="fv-row mb-6">
                                ...
                                <hr class="bg-secondary">
                                <div class="fv-row mb-6">
                                    <label class="form-label text-dark fs-7">{{__('Redemption Instructions')}}</label>
                                    <textarea class="form-control form-control-solid" type="text" rows="5" wire:model="redemption_instructions" placeholder="{{__('Redemption Instructions')}}"></textarea>
                                    @error('redemption_instructions')
                                    <span class="form-text text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                                <div class="fv-row mb-6">
                                    <label class="form-label text-dark fs-7">{{__('Card T & C\'s')}}</label>
                                    <textarea class="form-control form-control-solid" type="text" rows="5" wire:model="terms" placeholder="{{__('T & C\'s')}}"></textarea>
                                    @error('terms')
                                    <span class="form-text text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                                <div class="text-center my-10">
                                    <button type="submit" class="btn btn-success btn-block me-3 my-2" wire:loading.attr="disabled" wire:target="addCard">
                                        <span wire:loading.remove wire:target="addCard">{{__('Submit Card')}}</span>
    ...
    <div wire:ignore.self class="modal fade" id="export" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">{{__('Export Cards')}}</h3>
                    <div class="btn btn-icon btn-sm btn-active-light-success ms-2"
                ...
                <form wire:submit.prevent="save">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-block btn-dark" type="submit" wire:loading.attr="disabled" wire:loading.class="opacity-50" wire:target="save"><i class="bi bi-filetype-csv"></i>
        ...
        <div class="container">
            <div class="row g-xl-8 mb-6">
                ...
                <div class="col-md-6 text-end">
                    ...
                    <button id="kt_article_button" class="btn btn-dark me-4"><i class="bi bi-gift"></i> {{__('Add Card')}}</button>
            ...
            <div class="" wire:loading.class.delay="opacity-50" wire:target="search, status, orderBy, perPage">
                ...
                @endif
                <livewire:admin.country.edit-card :val=$val :admin=$admin :getCategory=$getCategory :wire:key="$val->id"></livewire:admin.country.edit-card>
                @endforeach
            ...
            <div class="text-center mt-20">
                <div class="symbol symbol-150px symbol-circle me-5 mb-10">
                    <div class="symbol-label fs-1 axora-dashboard-icon">
                        <i class="bi bi-gift" style="font-size:66px;"></i>
                    </div>
                </div>
                <h3 class="text-dark">{{__('No Gift Card Found')}}</h3>
                <p class="text-dark">{{__('We couldn\'t find any gift card')}}</p>
            </div>
            @endif
        </div>
    </div>
