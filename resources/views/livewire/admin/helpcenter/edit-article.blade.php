<div>
    <div wire:ignore.self id="kt_article_{{$val->id}}" class="bg-white" data-kt-drawer="true" data-kt-drawer-activate="true" data-kt-drawer-toggle="#kt_article_{{$val->id}}_button" data-kt-drawer-close="#kt_article_{{$val->id}}_close" data-kt-drawer-width="{'md': '700px'}">
        <div class="card w-100">
            <div class="card-header pe-5 border-0">
                ...
                <div class="card-toolbar">
                    <div class="btn btn-sm btn-icon btn-icon-dark btn-active-light-success" data-kt-drawer-dismiss="true" id="kt_article__{{$val->id}}_close">
                ...
                <div class="pb-5 mt-10 position-relative zindex-1">
                    <form class="form w-100 mb-10" wire:submit.prevent="update">
                        <div class="fv-row mb-6">
                            <input class="form-control form-control-solid" type="text" wire:model.defer="val.question" required placeholder="{{__('Question')}}" />
                            @error('val.question')
                            <span class="form-text text-danger">{{$message}}</span>
                            @enderror
                        </div>
                        <div class="fv-row mb-6">
                            <textarea class="form-control form-control-solid" rows="10" type="text" wire:model.defer="val.answer" required placeholder="{{__('Answer')}}"></textarea>
                            @error('val.answer')
                            <span class="form-text text-danger">{{$message}}</span>
                            @enderror
                        </div>
