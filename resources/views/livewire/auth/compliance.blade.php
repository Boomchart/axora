<div>
	<div class="axora-kyc-layout" wire:key="business-compliance-layout" x-data="{
			stage: @js($stage),
			link: @js($link),
			init() {
				$wire.on('updateStage', (data) => {
					this.stage = data;
					if(data == 'business_documents'){
			...
			navigateBack(data){
				this.link[this.stage] = false;
				$wire.set(`link.${this.stage}`, false);
				this.stage = data; 
				$wire.set('stage', data);
			}
		}">
		<aside class="axora-kyc-sidebar d-none d-md-flex">
			<div class="axora-kyc-sidebar-inner">
				<div class="axora-kyc-brand">
					...
					<p class="text-white fs-7 mb-0">{{ __('Complete your business verification to unlock full platform access.') }}</p>
				</div>

				<div class="axora-kyc-steps">
					<div class="axora-kyc-step {{ $stage == 'business_details' ? 'active' : '' }} {{ $link['business_details'] ? 'completed' : '' }}">
						<div class="axora-kyc-step-icon"><i class="bi bi-bank fs-4"></i></div>
						<div class="axora-kyc-step-content">
							<h6>{{ __('Business Details') }}</h6>
							<p>{{ __('Company profile and address') }}</p>
						</div>
						<i class="bi bi-check-circle-fill axora-kyc-step-check" x-show="link.business_details == true"></i>
					</div>

					<div class="axora-kyc-step {{ $stage == 'business_documents' ? 'active' : '' }} {{ $link['business_documents'] ? 'completed' : '' }}">
						<div class="axora-kyc-step-icon"><i class="bi bi-file-earmark-text fs-4"></i></div>
						<div class="axora-kyc-step-content">
							<h6>{{ __('Business Documents') }}</h6>
							<p>{{ __('Upload required documents') }}</p>
						</div>
						<i class="bi bi-check-circle-fill axora-kyc-step-check" x-show="link.business_documents == true"></i>
					</div>

					<div class="axora-kyc-step {{ $stage == 'business_directors' ? 'active' : '' }} {{ $link['business_directors'] ? 'completed' : '' }}">
						<div class="axora-kyc-step-icon"><i class="bi bi-people fs-4"></i></div>
						<div class="axora-kyc-step-content">
							<h6>{{ __('Business Directors') }}</h6>
							<p>{{ __('Add ownership details') }}</p>
						</div>
						<i class="bi bi-check-circle-fill axora-kyc-step-check" x-show="link.business_directors == true"></i>
					</div>
				</div>
			...
			<div class="axora-kyc-content">
				<form class="form w-100" wire:submit.prevent="next">
					...
					<div x-cloak x-show="stage === 'business_documents'" class="axora-kyc-panel">
						<div class="axora-kyc-header">
							<button type="button" x-on:click="navigateBack('business_details')" class="axora-kyc-back-link navigateBack"><i class="bi bi-arrow-left"></i>{{ __('Back') }}</button>
							<p><span class="axora-kyc-badge"><i class="bi bi-file-earmark-text"></i>{{ __('Document Upload') }}</span></p>
					...
					<div x-cloak x-show="stage === 'business_directors'" class="axora-kyc-panel">
						<div class="axora-kyc-header">
							<button type="button" x-on:click="navigateBack('business_documents')" class="axora-kyc-back-link navigateBack"><i class="bi bi-arrow-left"></i>{{ __('Back') }}</button>
							<p><span class="axora-kyc-badge"><i class="bi bi-people"></i>{{ __('Ownership Review') }}</span></p>
						...
						<div class="axora-director-summary">
							<div class="d-flex justify-content-between mb-2">
								...
								<button type="button" id="kt_director_button" class="btn btn-secondary btn-sm rounded-pill">
