            ...
            <div class="card-header pe-5 border-0">
                <div class="card-title">
                    <div class="d-flex justify-content-center flex-column me-3">
                        <div class="fs-4 text-gray-900 text-hover-success me-1 lh-1">{{__('Edit Template')}}</div>
                    </div>
                </div>
            ...
            <div class="card-body text-wrap">
                <div class="pb-5 mt-10 position-relative zindex-1">
                    <form class="form w-100 mb-10" wire:submit.prevent="update(Object.fromEntries(new FormData($event.target)))" method="post">
                        <p href="#" class="fs-7 text-dark mb-5">{{__('Allowed Tags')}}: @foreach(explode(',', $val->tags) as $tags) <span class="badge badge-warning badge-sm text-dark">{{$tags}}</span> @endforeach</p>
                        <div class="fv-row mb-6">
                            <input class="form-control form-control-solid" type="text" name="subject" required placeholder="{{__('Subject')}}" value="{{$val->subject}}"/>
                            @error('val.subject')
                            <span class="form-text text-danger">{{$message}}</span>
                            @enderror
                        </div>
                        <div class="fv-row mb-6" wire:ignore>
                            <textarea class="form-control form-control-solid tinymce{{$val->id}}" rows="10" type="text" name="body" placeholder="{{__('Details')}}">{{$val->body}}</textarea>
                            @error('val.body')
                            <span class="form-text text-danger">{{$message}}</span>
                            @enderror
                        </div>
...
<script>
    document.addEventListener('livewire:load', function() {
        initializeTinymce('textarea.tinymce{{$val->id}}');
    });
</script>
@endpush