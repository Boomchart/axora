<div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>{{__('Are you sure you want to delete this user, you can\'t undo this action?')}}</p>
                    @else
                    <p>{{__('Are you sure you want to delete this user permanently, you can\'t undo this action?')}}</p>
                    @endif
    ...
    <div wire:ignore.self class="modal fade" id="restore{{$val->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">{{__('Restore User')}}</h3>
                    <div class="btn btn-icon btn-sm btn-active-light-success ms-2" data-bs-dismiss="modal" aria-label="Close">
                        <span class="svg-icon svg-icon-1">
                            <i class="bi bi-x-lg fs-2"></i>
                        </span>
                ...
                <div class="modal-body">
                    <p>{{__('Are you sure you want to restore this user account')}}?</p>
                    <div class="text-center">
                        <button wire:click="restore" class="btn btn-danger btn-block" wire:loading.attr="disabled">
                            <span wire:loading.remove wire:target="restore">{{__('Restore')}}</span>
                            <span wire:loading wire:target="restore">{{__('Processing Request...')}}</span>
                        </button>
                    </div>
                </div>
            </div>
